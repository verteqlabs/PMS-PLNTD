...
import * as dataFormat from 'pages/CRUD/Patients/table/PatientsDataFormatters';
...
const PatientsTable = () => {
  ...
  const count = useSelector((store) => store.patients.list.count);
  const modalOpen = useSelector((store) => store.patients.list.modalOpen);
  const rows = useSelector((store) => store.patients.list.rows);
  const idToDelete = useSelector((store) => store.patients.list.idToDelete);
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Patients'
                : 'http://localhost:8080/api-docs/#/Patients'
            }
          >
