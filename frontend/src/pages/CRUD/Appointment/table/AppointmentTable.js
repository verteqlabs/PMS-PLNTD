...
import * as dataFormat from 'pages/CRUD/Appointment/table/AppointmentDataFormatters';

import actions from 'actions/appointment/appointmentListActions';
...
const AppointmentTable = () => {
  ...
  const count = useSelector((store) => store.appointment.list.count);
  const modalOpen = useSelector((store) => store.appointment.list.modalOpen);
  const rows = useSelector((store) => store.appointment.list.rows);
  const idToDelete = useSelector((store) => store.appointment.list.idToDelete);
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="appointment" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Appointment')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/appointment/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/appointment/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Appointment'
                : 'http://localhost:8080/api-docs/#/Appointment'
            }
          >
